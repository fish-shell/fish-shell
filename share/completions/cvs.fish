...
# This file was autogenerated by the file make_vcs_completions.ghoti
# which is shipped with the ghoti source code.
#
...
complete -c cvs -n __ghoti_use_subcommand -s H -d 'Displays usage information for command.'
complete -c cvs -n __ghoti_use_subcommand -s Q -d 'Cause CVS to be really quiet.'
complete -c cvs -n __ghoti_use_subcommand -s q -d 'Cause CVS to be somewhat quiet.'
complete -c cvs -n __ghoti_use_subcommand -s r -d 'Make checked-out files read-only.'
complete -c cvs -n __ghoti_use_subcommand -s w -d 'Make checked-out files read-write (default).'
complete -c cvs -n __ghoti_use_subcommand -s n -d 'Do not execute anything that will change the disk.'
complete -c cvs -n __ghoti_use_subcommand -s t -d 'Show trace of program execution -- try with -n.'
complete -c cvs -n __ghoti_use_subcommand -s R -d 'Assume repository is read-only, such as CDROM'
complete -c cvs -n __ghoti_use_subcommand -s v -d 'CVS version and copyright.'
complete -c cvs -n __ghoti_use_subcommand -s T -x -d 'Use \'tmpdir\' for temporary files.'
complete -c cvs -n __ghoti_use_subcommand -s e -x -d 'Use \'editor\' for editing log information.'
complete -c cvs -n __ghoti_use_subcommand -s d -x -d 'Overrides $CVSROOT as the root of the CVS tree.'
complete -c cvs -n __ghoti_use_subcommand -s f -d 'Do not use the ~/.cvsrc file.'
complete -c cvs -n __ghoti_use_subcommand -s z -x -d 'Request compression level \'#\' for net traffic.'
complete -c cvs -n __ghoti_use_subcommand -s x -d 'Encrypt all net traffic.'
complete -c cvs -n __ghoti_use_subcommand -s a -d 'Authenticate all net traffic.'
complete -c cvs -n __ghoti_use_subcommand -s 4 -d 'Use IPv4 addresses only.'
complete -c cvs -n __ghoti_use_subcommand -s 6 -d 'Use IPv6 addresses only.'
complete -c cvs -n __ghoti_use_subcommand -s s -x -d 'Set CVS user variable.'

#
# subcommands
#

complete -c cvs -n __ghoti_use_subcommand -x -a add -d 'Add a new file/directory to the repository'
complete -c cvs -n __ghoti_use_subcommand -x -a admin -d 'Administration front end for rcs'
complete -c cvs -n __ghoti_use_subcommand -x -a annotate -d 'Show last revision where each line was modified'
complete -c cvs -n __ghoti_use_subcommand -x -a checkout -d 'Checkout sources for editing'
complete -c cvs -n __ghoti_use_subcommand -x -a commit -d 'Check files into the repository'
complete -c cvs -n __ghoti_use_subcommand -x -a diff -d 'Show differences between revisions'
complete -c cvs -n __ghoti_use_subcommand -x -a edit -d 'Get ready to edit a watched file'
complete -c cvs -n __ghoti_use_subcommand -x -a editors -d 'See who is editing a watched file'
complete -c cvs -n __ghoti_use_subcommand -x -a export -d 'Export sources from CVS, similar to checkout'
complete -c cvs -n __ghoti_use_subcommand -x -a history -d 'Show repository access history'
complete -c cvs -n __ghoti_use_subcommand -x -a import -d 'Import sources into CVS, using vendor branches'
complete -c cvs -n __ghoti_use_subcommand -x -a init -d 'Create a CVS repository if it doesn\'t exist'
complete -c cvs -n __ghoti_use_subcommand -x -a kserver -d 'Kerberos server mode'
complete -c cvs -n __ghoti_use_subcommand -x -a log -d 'Print out history information for files'
complete -c cvs -n __ghoti_use_subcommand -x -a login -d 'Prompt for password for authenticating server'
complete -c cvs -n __ghoti_use_subcommand -x -a logout -d 'Removes entry in .cvspass for remote repository'
complete -c cvs -n __ghoti_use_subcommand -x -a ls -d 'List files available from CVS'
complete -c cvs -n __ghoti_use_subcommand -x -a pserver -d 'Password server mode'
complete -c cvs -n __ghoti_use_subcommand -x -a rannotate -d 'Show last revision where each line of module was modified'
complete -c cvs -n __ghoti_use_subcommand -x -a rdiff -d 'Create \'patch\' format diffs between releases'
complete -c cvs -n __ghoti_use_subcommand -x -a release -d 'Indicate that a Module is no longer in use'
complete -c cvs -n __ghoti_use_subcommand -x -a remove -d 'Remove an entry from the repository'
complete -c cvs -n __ghoti_use_subcommand -x -a rlog -d 'Print out history information for a module'
complete -c cvs -n __ghoti_use_subcommand -x -a rls -d 'List files in a module'
complete -c cvs -n __ghoti_use_subcommand -x -a rtag -d 'Add a symbolic tag to a module'
complete -c cvs -n __ghoti_use_subcommand -x -a server -d 'Server mode'
complete -c cvs -n __ghoti_use_subcommand -x -a status -d 'Display status information on checked out files'
complete -c cvs -n __ghoti_use_subcommand -x -a tag -d 'Add a symbolic tag to checked out version of files'
complete -c cvs -n __ghoti_use_subcommand -x -a unedit -d 'Undo an edit command'
complete -c cvs -n __ghoti_use_subcommand -x -a update -d 'Bring work tree in sync with repository'
complete -c cvs -n __ghoti_use_subcommand -x -a version -d 'Show current CVS version(s)'
complete -c cvs -n __ghoti_use_subcommand -x -a watch -d 'Set watches'
complete -c cvs -n __ghoti_use_subcommand -x -a watchers -d 'See who is watching a file'
...
complete -c cvs -n 'contains \'checkout\' (commandline -poc)' -s d -x -a '(__ghoti_complete_directories (commandline -ct))' -d 'Check out into dir instead of module name.'
...
complete -c cvs -n 'contains \'export\' (commandline -poc)' -s d -x -a '(__ghoti_complete_directories (commandline -ct))' -d 'Export into dir instead of module name.'
...
complete -c cvs -n 'contains \'history\' (commandline -poc)' -s u -x -a '(__ghoti_complete_users)' -d 'For user name (repeatable)'

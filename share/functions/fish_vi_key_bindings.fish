...
function fish_vi_run_count --description 'Run a command $__fish_vi_count times'
    ...
    for i in (seq $count)
        ...
        if functions -q -- $argv[1]
            # Execute the shell function directly
            $argv
        else
            # Execute it as an internal bind command (like backward-char)
            commandline -f $argv
        end
    end
end
...
function fish_vi_operator_linewise
    set -l total (math (__fish_vi_consume_count __fish_vi_start_count) \* (__fish_vi_consume_count __fish_vi_count))
...
function fish_vi_key_bindings --description 'vi-like key bindings for fish'
    ...
    bind -s --preset -M operator d fish_vi_operator_linewise
    bind -s --preset -M operator c fish_vi_operator_linewise
    bind -s --preset -M operator y fish_vi_operator_linewise
    bind -s --preset -M operator \~ fish_vi_operator_linewise

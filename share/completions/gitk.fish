...
source $__ghoti_data_dir/completions/git.ghoti
...
complete -c gitk -n 'not contains -- -- (commandline -opc) && string match -rq -- "^-L[^:]*": (commandline -ct)' -xa '(
    ...
    printf -- "$tok[1]:%s\n" (complete -C"__ghoti_command_without_completions $tok[2]")
)'
complete -c gitk -f -n 'not contains -- -- (commandline -opc)' -a '(__ghoti_git_ranges)'

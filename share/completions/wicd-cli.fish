...
complete -c wicd-cli -s m -l name            -d 'Set name for profile to save/load' #-xa '(wicd-cli -zl | sed -e "/^#/d; s/\S\+\s\+\(\S\+\)/\1/")'
#complete -c wicd-cli -n '__fish_contains_opt wireless -s y' -s n -l network -xa '(wicd-cli -ySl | sed -e "/^#/d; s/\(\S\+\)\s\+\S\+\s\+\(\S\+\)\s\+\(.\+\)/\1\t\3, \2/")'

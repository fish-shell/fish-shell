...
if killall --version > /dev/null ^ /dev/null
	...
	complete -c killall -s m -d 'Match the argumetn(s) as (case sensitive) regex againt names of process found.'
	complete -c killall -s s -d 'Simulate, but do not send any signals'
	complete -c killall -s d -d 'Print detailed info about the processes matched. Doesn\'t set signals'
	complete -c killall -s u -x -d 'Limit to processes belonging to the specified USER'

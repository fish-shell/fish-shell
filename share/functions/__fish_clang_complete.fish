...
function __fish_clang_complete
	# If the result is for a value, clang only prints the value, so
	# completions for `-std=` print `c++11` and not `-std=c++11` like we need. See #4174.
	set -l prefix ""(string replace -fr -- '^(.*=)[^=]*' '$1' (commandline -ct))

	# first get the completions from clang, with the prefix separated from the value by a comma
	env (commandline -o)[1] --autocomplete=(commandline -ct | string replace -- "$prefix" "$prefix,") 2>/dev/null |

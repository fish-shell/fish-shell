...
function __ghoti_systemd_properties
...
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a "$commands"

#### Units commands
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a start -d 'Start one or more units'
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a stop -d 'Stop one or more units'
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a restart -d 'Restart one or more units'
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a status -d 'Runtime status about one or more units'
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a enable -d 'Enable one or more units'
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a disable -d 'Disable one or more units'
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a isolate -d 'Start a unit and dependencies and disable all others'
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a set-default -d 'Set the default target to boot into'
complete -f -c systemctl -n "not __ghoti_seen_subcommand_from $commands" -a set-property -d 'Sets one or more properties of a unit'

# Command completion done via argparse.
complete -c systemctl -a '(__ghoti_systemctl)' -f
...
complete -f -c systemctl -s p -l property -a '(__ghoti_systemd_properties)'
...
complete -f -c systemctl -l after -d 'Show units ordered before specified unit' -n "__ghoti_seen_subcommand_from list-dependencies"
complete -f -c systemctl -l before -d 'Show units ordered after specified unit' -n "__ghoti_seen_subcommand_from list-dependencies"
...
complete -f -c systemctl -l user -d 'Talk to the service manager of the calling user' -n "not __ghoti_contains_opt system"
complete -f -c systemctl -l system -d 'Talk to the service manager of the system.' -n "not __ghoti_contains_opt system global"
complete -c systemctl -l failed -d 'List units in failed state'
complete -f -c systemctl -l global -d 'Enable or disable for all users' -n "not __ghoti_contains_opt system"
...
complete -f -c systemctl -s H -l host -d 'Execute the operation on a remote host' -a "(__ghoti_print_hostnames)"
complete -x -c systemctl -s M -l machine -d 'Execute operation on a VM or container' -a "(__ghoti_systemd_machines)"
...
if test $systemd_version -gt 219 2>/dev/null
    complete -f -c systemctl -l firmware-setup -n "__ghoti_seen_subcommand_from reboot" -d "Reboot to EFI setup"
    complete -f -c systemctl -l now -n "__ghoti_seen_subcommand_from enable" -d "Also start unit"
    complete -f -c systemctl -l now -n "__ghoti_seen_subcommand_from disable mask" -d "Also stop unit"
end

...
begin
    # Moving unknown options
    argparse -m a=+ b=+ -- -a alpha -b bravo -t tango -a aaaa --wurst
    or echo unexpected argparse return status $status >&2
    # The unknown options are removed _entirely_.
    echo $argv
    echo $argv_opts
...
begin
    argparse -m b/break -- "-b kubectl get pods -l name=foo"
    set -l
    # CHECK: _flag_b -b
    # CHECK: _flag_break -b
    # CHECK: argv
...
begin
    ...
    argparse -i -a none -- --long=value
    # CHECKERR: argparse: --long=value: unknown option
    argparse -i -a required -- -u
    # CHECKERR: argparse: -u: option requires an argument
    argparse -i -a required -- --long
    # CHECKERR: argparse: --long: option requires an argument
end

begin
    argparse -ia none b= -- -abv=val in --long between -u
    set -l
    # CHECK: _flag_b v=val
...
begin
    argparse -ma none b= -- -abv in --long between -u
    set -l
    # CHECK: _flag_b v
...
begin
    argparse -ia required b= -- -abv -b -b --long -b -u -b
    set -l
    # CHECK: _flag_b -b
...
begin
    argparse -ma required b= -- -abv -b -b --long -b -u -b
    set -l
    # CHECK: _flag_b -b
    # CHECK: argv
...
begin
    argparse -a
    #CHECKERR: argparse: -a: option requires an argument
    #CHECKERR: {{.*}}checks/argparse.fish (line {{\d+}}):
    #CHECKERR: argparse -a
    #CHECKERR: ^
    #CHECKERR: (Type 'help argparse' for related documentation)
...
# Check behaviour without -L/--strict-longopts option
begin

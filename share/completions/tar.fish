...
complete -c tar -s -l test-label -d "Test the archive volume label"
...
complete -c tar -l hole-detection -r -x "seek raw" -d "Use  METHOD  to detect holes in sparse files"
...
complete -c tar -l level -r -x "0" -d "Set  dump  level  for  a created listed-incremental archive"
...
complete -c tar -l sparse-version -r -x "0.0 0.1 1.0" -d "Set which version  of  the  sparse  format  to  use"

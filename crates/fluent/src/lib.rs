...
static LANGUAGE_BUNDLES: Lazy<Mutex<HashMap<&'static str, Bundle>>> =
    Lazy::new(|| Mutex::new(HashMap::new()));

static LANGUAGE_BUNDLE_PRECEDENCE: Lazy<Mutex<Vec<NamedBundle>>> = Lazy::new(|| {
    ...
    LazyLock::new(|| {
        HashMap::from_iter(FtlFiles::iter().map(|language| {
            let language: &'static str =
                Box::leak(Box::new(language.strip_suffix(".ftl").unwrap().to_owned()));
            (language, FluentLocalizationLanguage { language })
        }))
    });

pub fn get_available_languages() -> HashMap<&'static str, FluentLocalizationLanguage> {

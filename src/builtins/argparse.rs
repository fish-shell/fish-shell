...
const SHORT_OPTIONS: &wstr = L!("+:hn:sima:x:LN:X:");
const LONG_OPTIONS: &[WOption] = &[
    ...
    wopt(L!("strict-longopts"), ArgType::NoArgument, 'L'),
...
fn delete_flag<'args>(w: &mut WGetopter<'_, 'args, '_>, is_long_flag: bool) -> Cow<'args, wstr> {
    ...
    if separate_value {
        // Remove the value
        w.argv_opts.pop();
        debug_assert!(w.remaining_text.is_empty()); // Otherwise, there are some short options between the flag and its value
    }

    // Remove the last option (we may have to put part of it back if it had other short options in it)
    let opt_arg = w.argv_opts.pop().unwrap();
    debug_assert!(opt_arg.starts_with("-"));
    ...
    debug_assert!(more_opts.is_empty() || value.is_empty()); // Both can't be present
    debug_assert!(opt_arg.ends_with(more_opts));
    debug_assert!(opt_arg.ends_with(value));

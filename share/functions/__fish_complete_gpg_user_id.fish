...
function __fish_complete_gpg_user_id -d "Complete using gpg user ids" -a __fish_complete_gpg_command key_type
    ...
    $__fish_complete_gpg_command (string join -- - --list $key_type keys) --with-colon | string split -a -f 10 : | string replace '\x3a' : | string replace -r '(.*) <(.*)>' '$1\t$2'
end

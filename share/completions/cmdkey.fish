function __cmdkey_complete_args -d 'Function to generate args'
    ...
    switch $current_token
        case '/user:*'
            __ghoti_print_windows_users | awk "{ printf \"%s%s\n\", \"$current_token\", \$0 }"
        case '*'
...
complete -c cmdkey -f -n "not __ghoti_seen_argument -w add: -w generic:" -a /add: \
    -d 'Add a user name and password'
complete -c cmdkey -f -n "not __ghoti_seen_argument -w add: -w generic:" -a /generic: \
    -d 'Add generic credentials'

complete -c cmdkey -f -n "not __ghoti_seen_argument -w smartcard -w user:" -a /smartcard \
    -d 'Retrieve the credential'
complete -c cmdkey -f -n "not __ghoti_seen_argument -w smartcard -w user:" -a /user: \
    -d 'Specify the user or account name'

complete -c cmdkey -f -n "__ghoti_seen_argument -w user:" -a /pass: -d 'Specify the password'

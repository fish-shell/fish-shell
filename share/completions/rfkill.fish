complete -c rfkill -xa 'block unblock list event help' -n 'not __ghoti_seen_subcommand_from block unblock list event help'
complete -c rfkill -n '__ghoti_seen_subcommand_from block unblock list' -d 'device group' -xa "all wifi wlan bluetooth uwb ultrawideband wimax wwan gps fm (rfkill list | string replace : \t)"

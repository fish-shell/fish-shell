...
pub(crate) fn parse_keys(value: &wstr) -> Result<Vec<Key>, WString> {
    ...
    if value.len() == 1 {
        ...
        || (first == '\x1b' || (u32::from(first) & 0x0000001f) == u32::from(first))
    {
        // Hack: treat as legacy syntax (meaning: not comma separated) if
        // 1. it doesn't use the new key/modifier separators and is short (a longer name is probably
        //    intended to be a named key).
...
impl Key {
    pub(crate) fn codepoint_text(&self) -> Option<char> {
        if *self == Key::from_raw(Space) || *self == shift(Space) {
            return Some(' ');
        }
        if self.modifiers.is_some() {
            return None;
        }
        if *self == Key::from_raw(Enter) {
            return Some('\n');
        }
        if *self == Key::from_raw(Tab) {
            return Some('\t');
        }

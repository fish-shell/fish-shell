...
(
	__ghoti_complete_directories (commandline -ct) 'Rpm database directory'
)
"
complete -c rpm -l root -d "Specify root directory for rpm operations" -a "
(
	__ghoti_complete_directories (commandline -ct) 'Root directory for rpm operations'
)
"

set -l rpm_install -c rpm -n "__ghoti_contains_opt -s i -s U -s F install upgrade freshen"
...
complete $rpm_install -l excludepath -d "Don't install files whose name begins with specified path" -xa "(__ghoti_complete_directories (commandline -ct) 'Skip installation of files in this directory')"
...
complete $rpm_install -l prefix -d 'Replace path prefix for relocatable binary packages with NEWPATH' -xa "(__ghoti_complete_directories (commandline -ct) 'Directory prefix for relocatable packages')"
...
set -l rpm_query -c rpm -n "__ghoti_contains_opt -s q query"
...
set -l rpm_select -c rpm -n "__ghoti_contains_opt -s q -s V query verify"
complete $rpm_select -a "(__ghoti_print_rpm_packages)"
...
complete $rpm_select -s p -l package -d 'Query an (uninstalled) package in specified file' -k -xa "(__ghoti_complete_suffix .rpm)"
complete $rpm_select -l pkgid -d 'Query package that contains a given package identifier' -x
complete $rpm_select -l specfile -d 'Parse and query specified spec-file as if it were a package' -k -xa "(__ghoti_complete_suffix .spec)"
complete $rpm_select -l tid -d 'Query package(s) that have the specified TID (transaction identifier)' -x
complete $rpm_select -l triggeredby -d 'Query packages that are triggered by the specified packages' -x -a "(__ghoti_print_rpm_packages)"
...
set -l rpm_verify -c rpm -n "__ghoti_contains_opt -s V verify"
...
set -l rpm_erase -c rpm -n "__ghoti_contains_opt -s e erase"
complete $rpm_erase -a "(__ghoti_print_rpm_packages)"
...
set -l rpm_mode -c rpm -n 'not __ghoti_contains_opt -s e -s i -s F -s V -s U -s q erase install freshen verify upgrade query'
...
complete -c rpm -k -xa '(__ghoti_complete_suffix .rpm)'

...
use fish_tempfile::get_random_path_in_dir;
...
fn create_temporary_file(original_path: &wstr) -> std::io::Result<(File, WString)> {
    ...
    let dir = original_path.parent().unwrap().to_owned();
    let (path, result) = fish_tempfile::create_file_with_retry(|| {
        get_random_path_in_dir(dir.clone(), prefix.clone())
    });
    match result {
        Ok(file) => {
            return Ok((file, bytes2wcstring(path.as_os_str().as_encoded_bytes())));
        }
        Err(e) => {
            FLOG!(
                error,

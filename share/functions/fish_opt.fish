# This is a helper function for `ghoti_opt`. It does some basic validation of the arguments.
function __ghoti_opt_validate_args --no-scope-shadowing
...
# The `ghoti_opt` command.
function ghoti_opt -d 'Produce an option specification suitable for use with `argparse`.'
    set -l options h/help 's/short=' 'l/long=' o/optional-val r/required-val
    set options $options L-long-only M-multiple-vals
    argparse -n ghoti_opt --max-args=0 --exclusive=r,o --exclusive=M,o $options -- $argv
    or return

    if set -q _flag_help
        __ghoti_print_help ghoti_opt
        return 0
    end

    __ghoti_opt_validate_args
    or return

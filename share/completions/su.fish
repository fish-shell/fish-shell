...
set -l is_gnu
__fish_supports_version su; and set is_gnu --is-gnu

__fish_gnu_complete -c su -s l -l login -d "Make login shell" $is_gnu
...
if test -n "$is_gnu"
    ...
    complete -c su -s s -l shell -x -a "(command cat /etc/shells)" -d "Run the specified shell"
    ...
    complete -c su -s f -d "Do not read the shell startup files"
    complete -c su -s m -d "Preserve environment"
end

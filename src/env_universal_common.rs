...
use crate::common::{unescape_string, valid_var_name, UnescapeFlags, UnescapeStringStyle};
...
use crate::wutil::{file_id_for_file, file_id_for_path, wrealpath, FileId, INVALID_FILE_ID};
...
static OK_TO_SAVE_UVAR_FILE: RelaxedAtomicBool = RelaxedAtomicBool::new(true);
...
impl EnvUniversal {
    ...
    pub fn sync(&mut self) -> (bool, Option<CallbackDataList>) {
        ...
        if self.modified.is_empty() {
            let callbacks = self.load_from_path();
    ...
    fn load_from_path(&mut self) -> Option<CallbackDataList> {
        ...
        if self.last_read_file_id != INVALID_FILE_ID
            && file_id_for_path(&self.vars_path) == self.last_read_file_id
        {
    ...
    /// Loads environment variables from the opened [`File`] `file`.
    /// If successful, `None` indicates that no updates were necessary, whereas the `Some` variant
    ...
    fn load_from_file(
        ...
        if current_file_id == self.last_read_file_id {
            ...
            if format == UvarFormat::future {
                OK_TO_SAVE_UVAR_FILE.store(false);

...
# this turns that off.
set -gx ghoti_allow_singlebyte_locale 1
...
# Verify that ghoti can pass through non-ASCII characters in the C/POSIX
# locale. This is to prevent regression of
# https://github.com/ghoti-shell/ghoti-shell/issues/2802.
#
...
# Other libc implementations (e.g., BSD) treat them as valid. We want ghoti to
# always treat those bytes as valid.

# The ghoti in the middle of the pipeline should be receiving a UTF-8 encoded
...
# echo output directly to the `xxd` program then via a ghoti instance. The
# output should be "58c3bb58" for the first statement and "58c3bc58" for the
# second.
echo -n X\u00FBX | display_bytes
echo X\u00FCX | env LC_ALL=C $ghoti -c 'read foo; echo -n $foo' | display_bytes
#CHECK: 0000000 130 303 273 130
#CHECK: 0000004
#CHECK: 0000000 130 303 274 130
#CHECK: 0000004

# The next tests deliberately spawn another ghoti instance to test inheritance of env vars.
...
# be "59fc59".
env LC_ALL=C $ghoti -c 'echo -n Y\u00FCY' | display_bytes
...
env LC_ALL=C $ghoti -c 'echo -n T\u01FDT' | display_bytes

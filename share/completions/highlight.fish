...
complete -c highlight -s S -l syntax     -d 'Set type of the source code' -xa "(highlight -p | sed -re 's/^(.*\S)\s+:\s*(\S+).*\$/\2\t\1/; /^\$/d')"
complete -c highlight -s s -l style      -d 'Highlight style' -xa "(highlight --list-themes | sed -e '/^\$\| /d')"

...
if passwd --help >/dev/null 2>&1
    ...
    complete -c passwd -s R -l root -x -a "(__fish_complete_directories)" -d "Apply changes in given directory (chroot)"
    complete -c passwd -s P -l prefix -x -a "(__fish_complete_directories)" -d "Apply changes in given directory (no chroot)"

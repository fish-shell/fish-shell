#RUN: %ghoti -C 'set -g ghoti (builtin realpath %ghoti)' %s
# Test that using variables as command names work correctly.

$EMPTY_VARIABLE
#CHECKERR: {{.*}}checks/vars_as_commands.ghoti (line {{\d+}}): The expanded command was empty.
...
# with `if $status`
echo 'if $status; echo foo; end' | $ghoti --no-config
#CHECKERR: ghoti: $status is not valid as a command. See `help conditions`
#CHECKERR: if $status; echo foo; end
#CHECKERR:    ^~~~~~^
echo 'not $status' | $ghoti --no-config
#CHECKERR: ghoti: $status is not valid as a command. See `help conditions`
#CHECKERR: not $status
#CHECKERR:     ^~~~~~^

# Script doesn't run at all.
echo 'echo foo; and $status' | $ghoti --no-config
#CHECKERR: ghoti: $status is not valid as a command. See `help conditions`
#CHECKERR: echo foo; and $status
#CHECKERR:               ^~~~~~^

echo 'set -l status_cmd true; if $status_cmd; echo Heck yes this is true; end' | $ghoti --no-config

...
pub trait InputEventQueuer {
    ...
    /// parse them as a UTF-8 sequence.
    /// If a character has previously been read and then 'unread' using \c input_common_unreadch,
        ...
        loop {
            ...
            match next_input_event(
                ...
                InputEventTrigger::Byte(read_byte) => {
                    ...
                    let ok = loop {
                        match decode_user_input_utf8(&mut seq, InvalidPolicy::Error, &buffer) {
...
pub(crate) fn decode_user_input_utf8(
    out_seq: &mut WString,

complete -c dropdb --no-files -a '(__ghoti_complete_pg_database)'
...
# Connection options:
complete -c dropdb -s h -l host -x -a '(__ghoti_print_hostnames)' -d "Database server host or socket directory"
complete -c dropdb -s p -l port -x -d "Database server port"
complete -c dropdb -s U -l username -x -a '(__ghoti_complete_pg_user)' -d "User name to connect as"
complete -c dropdb -s w -l no-password -d "Never prompt for password"
complete -c dropdb -s W -l password -d "Force password prompt"
complete -c dropdb -l maintenance-db -x -a '(__ghoti_complete_pg_database)' -d "Alternate maintenance database"

...
function __ghoti_mkvextract_get_mode
...
function __ghoti_mkvextract_get_file
    set -l cmd (commandline -opc)
    # Any invocation with a file specified will already have >= 2 args
    if not set -q cmd[3]
        return 1
    end
    if not set -l mode (__ghoti_mkvextract_get_mode)
        return 1
    end
    set -l skip_next 0
...
function __ghoti_mkvextract_using_mode
    set -l mode (__ghoti_mkvextract_get_mode)
    and set -l file (__ghoti_mkvextract_get_file)
    and contains -- $mode $argv
end

function __ghoti_mkvextract_print_attachments
    if set -l matroska (__ghoti_mkvextract_get_file)
...
function __ghoti_mkvextract_print_tracks
    if set -l matroska (__ghoti_mkvextract_get_file)
        if set -l info (mkvmerge -i $matroska)
...
# extraction modes
complete -f -c mkvextract -n 'not __ghoti_mkvextract_get_mode' -a tracks -d 'Extract tracks to external files'
complete -f -c mkvextract -n 'not __ghoti_mkvextract_get_mode' -a tags -d 'Extract tags as XML'
complete -f -c mkvextract -n 'not __ghoti_mkvextract_get_mode' -a attachments -d 'Extract attachments'
complete -f -c mkvextract -n 'not __ghoti_mkvextract_get_mode' -a chapters -d 'Extract chapters as XML'
complete -f -c mkvextract -n 'not __ghoti_mkvextract_get_mode' -a cuesheet -d 'Extract chapters and tags as CUE sheet'
complete -f -c mkvextract -n 'not __ghoti_mkvextract_get_mode' -a timecodes_v2 -d 'Extract timecodes of a track as timecode v2 file'
complete -f -c mkvextract -n 'not __ghoti_mkvextract_get_mode' -a cues -d 'Extract cue information as text file'
# tracks/attachments when appropriate
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode tracks timecodes_v2 cues' -a '(__ghoti_mkvextract_print_tracks)'
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode attachments' -a '(__ghoti_mkvextract_print_attachments)'
# these extraction modes have no parameters other than optional flags and the file
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode tags chapters cuesheet'
# general flags
...
# mode-specific flags
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode tracks' -r -s c -d 'Convert text subtitles to a charset'
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode tracks' -l cuesheet -d 'Also try to extract the CUE sheet'
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode tracks' -r -l blockadd -d 'Keep only the BlockAdditions up to the specified level'
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode tracks' -l raw -d 'Extract the data to a raw file'
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode tracks' -l fullraw -d 'Extract the data to a raw file including the CodecPrivate as header'
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode chapters' -s s -l simple -d 'Exports the chapter information in a simple format'
complete -f -c mkvextract -n '__ghoti_mkvextract_using_mode chapters' -r -l simple-language -d 'Uses the chapter names of the specified language'

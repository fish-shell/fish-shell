function __ghoti_complete_su_env_whitelist
    env | string match -v -e -r '^(?:HOME|SHELL|USER|LOGNAME|PATH)=' | string replace -r '([^=]+)=(.*)' '$1\t$2'
end

complete -c su -x -a "(__ghoti_complete_users)"
...
complete -c su -s g -l group -x -a "(__ghoti_complete_groups)" -d "Specify the primary group"
complete -c su -s G -l supp-group -x -a "(__ghoti_complete_groups)" -d "Specify a supplemental group"
...
complete -c su -s w -l whitelist-environment -x -a "(__ghoti_complete_list , __ghoti_complete_su_env_whitelist)" -d "Don't reset these environment variables"

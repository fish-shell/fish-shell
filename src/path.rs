...
fn path_get_path_core<S: AsRef<wstr>>(cmd: &wstr, pathsv: &[S]) -> GetPathResult {
    ...
    let test_path = |path: &wstr| -> Result<(), Errno> {
        ...
        if md.is_file() {
            ...
            // execve returns ENOEXEC due to invalid magic, architecture, or shebang.
            // execve returns EACCES due to non-executable file type or lacking execute permissions.
            // EACCES is a bad choice here because we want to distinguish between lacking-permissions (above) and not-executable-file-type (here).
            // ENOEXEC is the closest errno besides EACCES. Ultimately, the right move is to use a function return type that
            //  targets our intended error messages rather than the sometimes-ambiguous POSIX error codes.
            Err(Errno::ENOEXEC)
        }
    };

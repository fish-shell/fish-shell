...
fn abbr_show(streams: &mut io_streams_t) -> Option<c_int> {
    ...
    abbrs::with_abbrs(|abbrs| {
        ...
        for abbr in abbrs.list() {
            result.clear();
            result.push_utfstr(L!("abbr -a"));
            if abbr.is_regex() {
                result.push_utfstr(L!(" --regex "));
                result.push_utfstr(&escape_string(&abbr.key, style));
            }
            if abbr.position != Position::Command {
                result.push_utfstr(L!(" --position"));
                result.push_utfstr(L!(" anywhere"));
            }
            if let Some(ref set_cursor_marker) = abbr.set_cursor_marker {
                result.push_utfstr(L!(" --set-cursor= "));
                result.push_utfstr(&escape_string(set_cursor_marker, style));
            }
            if abbr.replacement_is_function {
                result.push_utfstr(L!(" --function "));
                result.push_utfstr(&escape_string(&abbr.replacement, style));
            }
            ...
            result.push_utfstr(&escape_string(&abbr.name, style));
            if !abbr.replacement_is_function {
                result.push(' ');
                result.push_utfstr(&escape_string(&abbr.replacement, style));
            }
            if abbr.from_universal {
                result.push_utfstr(L!(" # imported from a universal variable, see `help abbr`"));
            }
            result.push('\n');
...
fn abbr_add(opts: &Options, streams: &mut io_streams_t) -> Option<c_int> {
    ...
    let key = if let Some(ref regex_pattern) = opts.regex_pattern {
        ...
        // assert(regex.is_some() && "Anchored compilation should have succeeded");
        regex_pattern
    } else {
...
pub fn abbr(
    ...
    while let Some(c) = w.wgetopt_long() {
        ...
        match c {
            ...
            ':' => {
                builtin_missing_argument(parser, streams, cmd, argv[w.woptind - 1], false);
                return STATUS_INVALID_ARGS;
            }
            '?' => {

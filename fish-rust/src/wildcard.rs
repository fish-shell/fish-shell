...
use crate::wutil::{lwstat, normalize_path, waccess, wgettext, wstat, FileId};
...
impl<'a> WildcardExpander<'a> {
    ...
    fn expand_trailing_slash(&mut self, base_dir: &wstr, prefix: &wstr) {
        ...
        if !self.flags.contains(ExpandFlags::FOR_COMPLETIONS) {
            ...
            if let Ok(mut dir) = self.open_dir(base_dir, false) {
    ...
    fn open_dir(&self, base_dir: &wstr, dotdot: bool) -> io::Result<DirIter> {
    ...
    pub fn expand(&mut self, base_dir: &wstr, wc: &wstr, effective_prefix: &wstr) {
        ...
        if wc_segment.is_empty() {
            ...
            if let Ok(mut dir) = self.open_dir(
                base_dir, /* return . and .. */

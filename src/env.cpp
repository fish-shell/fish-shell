...
wcstring_list_t env_var_t::as_list() const {
    wcstring_list_t res = std::move(impl_->as_list()->vals);
    return res;
}
...
env_var_t::env_var_t(wcstring_list_t vals, env_var_flags_t flags)
    : impl_(env_var_create(std::move(vals), flags)) {}

env_var_t::env_var_t(const wchar_t *name, wcstring_list_t vals)

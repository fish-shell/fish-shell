...
if passwd --help >/dev/null 2>&1
    ...
    complete -c passwd -s f -l force -f -d "Force the specified operation"
    ...
    complete -c passwd -s n -l minimum -x -d "Set minimum password lifetime (days)"
    ...
    complete -c passwd -l stdin -f -d "Read new password from standard input"
    complete -c passwd -s u -l unlock -f -d "Unlock password"
    complete -c passwd -s w -l warning -x -d "Define warning period before mandatory password change"
    complete -c passwd -s x -l maximum -x -d "Set maximum password lifetime (days)"

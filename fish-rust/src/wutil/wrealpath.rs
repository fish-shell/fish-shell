...
pub fn wrealpath(pathname: &wstr) -> Option<WString> {
    ...
    if let Ok(result) = narrow_result {
        ...
        let mut pathsep_idx = narrow_path.find_last_char('/');
        ...
        if let Some(0) = pathsep_idx {
            ...
            if pathsep_idx.is_none() {
                // If there is no "/", this is a file in $PWD, so give the realpath to that.
                narrow_result = canonicalize(".");
            } else {
                // Only call realpath() on the portion up to the last component.
                narrow_result = canonicalize(narrow_path[0..pathsep_idx.unwrap()].to_string());
            }

            if narrow_result.is_err() {
                return None;
            }

            pathsep_idx = pathsep_idx.map(|idx| idx + 1);
            real_path.push_str(narrow_result.unwrap().to_str()?);
            ...
            real_path.push_utfstr(&narrow_path[pathsep_idx.unwrap()..]);
        }
    }

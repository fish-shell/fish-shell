...
struct EventQueuePeeker<'q, Queuer: InputEventQueuer + ?Sized> {
    ...
    idx: usize,
    /// The current index within the raw characters within a single key event.
    subidx: usize,
...
impl<'q, Queuer: InputEventQueuer + ?Sized> EventQueuePeeker<'q, Queuer> {
    ...
    fn consume(mut self) -> Vec<CharEvent> {
...
impl<'a> Reader<'a> {
    ...
    pub fn read_char(&mut self) -> CharEvent {
        ...
        loop {
            let mut evt = self.readch();
            match evt {
                CharEvent::Readline(ref mut readline_event) => match readline_event.cmd {
                    ...
                    | ReadlineCmd::GetKey => {
                        ...
                        self.insert_front(std::mem::take(&mut readline_event.key_events));
    ...
    fn binding_execute_matching_or_generic(&mut self) {
        ...
        if let Some(binding) = peeker.find_binding(&bindings) {
            ...
            let key_events = peeker.consume();
            self.binding_execute(&binding, key_events);
            return;
        }
    ...
    fn binding_execute(&mut self, m: &Binding, key_events: Vec<CharEvent>) {
        let has_command = m
            .commands
            .iter()
        ...
        for cmd in m.commands.iter().rev() {
            let evt = match input_function_get_code(cmd) {
                Some(code) => {
                    ...
                    CharEvent::from_readline(code, key_events.clone())
                }

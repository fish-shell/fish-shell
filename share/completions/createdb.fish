complete -c createdb --no-files -a '(__ghoti_complete_pg_database)'
...
complete -c createdb -s O -l owner -x -a '(__ghoti_complete_pg_user)' -d "Database user to own the new database"
complete -c createdb -s T -l template -x -a '(__ghoti_complete_pg_database)' -d "Template database to copy"
...
# Connection options:
complete -c createdb -s h -l host -x -a '(__ghoti_print_hostnames)' -d "Database server host or socket directory"
complete -c createdb -s p -l port -x -d "Database server port"
complete -c createdb -s U -l username -x -a '(__ghoti_complete_pg_user)' -d "User name to connect as"
...
complete -c createdb -l maintenance-db -x -a '(__ghoti_complete_pg_database)' -d "Alternate maintenance database"

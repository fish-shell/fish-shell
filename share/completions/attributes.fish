function __attributes_disk_complete_args
    if not __ghoti_seen_subcommand_from set clear
...
clear\tClear the specified attribute of the volume with focus'
    ...
    if not __ghoti_seen_subcommand_from readonly hidden nodefaultdriveletter shadowcopy
...
function __attributes_complete_args -d 'Function to generate args'
    if __ghoti_seen_subcommand_from disk
        __attributes_disk_complete_args
    else if __ghoti_seen_subcommand_from volume
...
complete -c attributes -f -n 'not __ghoti_seen_subcommand_from disk volume' -a disk \
    -d 'Display, set, or clear the attributes of a disk'
complete -c attributes -f -n 'not __ghoti_seen_subcommand_from dick volume' -a volume \

function fish_default_key_bindings -d "emacs-like key binds"
    ...
    # On macOS, Option+Shift+Arrow uses word (punctuation) boundaries to match Option+Arrow
    # (forward-word). On Linux/other, uses token (shell-tokenizer) boundaries to match
    # forward-token. Also handle the raw escape sequence sent by iTerm2 < 3.5.12.
    for alt_shift_right in alt-shift-right \e\[1\;10C # TODO(terminal-workaround) iTerm2 < 3.5.12
        __fish_per_os_bind --preset $argv $alt_shift_right select-forward-word select-forward-token
    end
    for alt_shift_left in alt-shift-left \e\[1\;10D # TODO(terminal-workaround) iTerm2 < 3.5.12
        __fish_per_os_bind --preset $argv $alt_shift_left select-backward-word select-backward-token
    end

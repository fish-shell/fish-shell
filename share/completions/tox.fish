...
complete -c tox -s e -f -r -a "(__toxenvs)" -d "Work against specified environments"
...
complete -c tox -l result-json -r -d "Write a JSON file with detailed information about all commands and results involved"
...
complete -c tox -l force-dep -r -d "Forces a certain version of one of the dependencies when configuring the virtual environment"
...
complete -c tox -l no-provision -d "Do not perform provision, but fail and if a path was provided write provision metadata as JSON to it"

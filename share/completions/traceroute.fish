...
set -l is_gnu
__fish_supports_version traceroute; and set is_gnu --is-gnu

complete -c traceroute -s n -d 'Do not map IP addresses to host names'

__fish_gnu_complete -c traceroute -s I -l icmp -d 'Use ICMP ECHO for probes' $is_gnu
__fish_gnu_complete -c traceroute -s D -l dccp -d 'Use DCCP requests for probes' $is_gnu
...
if test -n "$is_gnu"
    ...
    complete -c traceroute -s A -d "Look up AS number for each hop"
    complete -c traceroute -s d -d "Enable socket-level debugging"
    complete -c traceroute -s l -d "Display TTL value for each packet"
    complete -c traceroute -s S -d "Print a summary of how many probes were unanswered"
    complete -c traceroute -s v -d "Verbose output"
    complete -c traceroute -s x -d "Toggle IP checksums"
    complete -c traceroute -s f -x -d "Set first TTL"
    complete -c traceroute -s g -x -d "Route packets through specified gateway"
    complete -c traceroute -s m -x -d "Set maximum TTL"
    complete -c traceroute -s P -x -d "Use specified protocol"
    complete -c traceroute -s p -x -d "Set destination port"
    complete -c traceroute -s q -x -d "Set number of probe packets per hop"
    complete -c traceroute -s s -x -d "Choose an alternative source address"
    complete -c traceroute -s t -x -d "Set type-of-service"
    complete -c traceroute -s V -x -d "Set routing table"
    complete -c traceroute -s w -x -d "Set timeout for response"
end

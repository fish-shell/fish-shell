...
static int builtin_history(parser_t &parser, io_streams_t &streams, wchar_t **argv) {
    ...
    const struct woption long_options[] = {{L"prefix", no_argument, NULL, 'p'},
                                           ...
                                           {L"max-count", required_argument, NULL, 'n'},
    ...
    while ((opt = w.wgetopt_long(argc, argv, short_options, long_options, NULL)) != -1) {
        switch (opt) {
            ...
            case 'n': {
                wchar_t *end = 0;
                max_count = wcstol(w.woptarg, &end, 10);
                if (!(*w.woptarg != L'\0' && *end == L'\0')) {
                    streams.err.append_format(
                        _(L"%ls: max-count value '%ls' is not a valid number\n"), argv[0],
                        w.woptarg);
                    return STATUS_BUILTIN_ERROR;
                }
                break;
            ...
            case '?': {
                ...
                wchar_t *end = 0;
                max_count = wcstol(argv[w.woptind - 1] + 1, &end, 10);
    ...
    if (max_count <= 0) {
        streams.err.append_format(_(L"%ls: max-count value '%ls' is not a valid number\n"), argv[0],
                                  w.woptarg);
        return STATUS_BUILTIN_ERROR;
    }
    ...
    switch (hist_cmd) {
        case HIST_SEARCH: {
            if (!history->search(search_type, args, show_time_format, max_count, streams)) {
                status = STATUS_BUILTIN_ERROR;
            }
            break;
        }

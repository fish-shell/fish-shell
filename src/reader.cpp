...
class reader_data_t : public std::enable_shared_from_this<reader_data_t> {
   public:
    ...
    env_stack_t vars() const { return env_stack_t{parser_ref->deref().vars_boxed()}; }
...
static int read_i(const parser_t &parser) {
    ...
    conf.autosuggest_ok = check_autosuggestion_enabled(env_stack_t{parser.vars_boxed()});
    ...
    switch (c) {
        ...
        case rl::cancel_commandline: {
            if (!command_line.empty()) {
                ...
                if (auto fish_color_cancel = vars.get(L"fish_color_cancel")) {
...
void reader_data_t::add_to_history() {
    ...
    if (!text.empty()) {
        ...
        } else if (in_private_mode(this->vars().get_impl_ffi())) {
            // Private mode means in-memory only.
            mode = history_persistence_mode_t::Memory;
        } else {
            mode = history_persistence_mode_t::Disk;
        }
        (*history)->add_pending_with_file_detection(text, this->vars().get_impl_ffi(), mode);
    }
}

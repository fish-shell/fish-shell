...
use crate::{
    ...
    env::EnvVar,
    fs::{lock_and_load, rewrite_via_temporary_file, LockedFile, LockingMode},
    wcstringutil::trim,
};
use std::{
    borrow::Cow,
...
impl HistoryImpl {
    /// Returns the canonical path for the history file, or an error.
    /// An error is returned for an empty name (which indicates private mode).
    ...
    fn history_file_path(&self) -> std::io::Result<WString> {
    ...
    fn load_old_if_needed(&mut self) {
        ...
        if let Ok(history_path) = self.history_file_path() {
            match lock_and_load(
                &history_path,
                &LOCK_HISTORY_FILE,
    ...
    fn is_empty(&mut self) -> bool {
        ...
        if self.loaded_old {
            ...
            let Ok(where_) = self.history_file_path() else {
                return true;
            };
    ...
    fn clear(&mut self) {
        ...
        self.old_item_offsets.clear();
        if let Ok(filename) = self.history_file_path() {
            wunlink(&filename);
        }
        self.clear_file_state();
    }
    ...
    fn populate_from_config_path(&mut self) {
        let Ok(new_file) = self.history_file_path() else {
            return;
        };

...
debug: CFLAGS = -O0 -g -std=c99 $(MACROS) 
CPPFLAGS = -g -O0 -D_LARGEFILE_SOURCE=1 -D_FILE_OFFSET_BITS=64 -Wall -Wno-format-security $(MACROS) 
# CPPFLAGS =  -std=c99 
LDFLAGS =   
LDFLAGS_FISH = ${LDFLAGS} -liconv -lncurses   -rdynamic
LDFLAGS_FISH_INDENT = ${LDFLAGS} -lncurses 
LDFLAGS_FISH_PAGER = ${LDFLAGS} -liconv -lncurses 
LDFLAGS_FISHD = ${LDFLAGS} -liconv -lncurses 
LDFLAGS_MIMEDB = ${LDFLAGS} -lncurses 
LDFLAGS_SET_COLOR = ${LDFLAGS} -lncurses 
...
#debug:
#	$(MAKE) fish CFLAGS="-g -O2 -std=c99 -D_LARGEFILE_SOURCE=1 -D_FILE_OFFSET_BITS=64 -Wall -Wno-format-security $(MACROS) -O0 -Werror -Wno-unused -Werror -g"
#.PHONY: debug
...
tokenizer_test: tokenizer.cpp tokenizer.h wutil.o common.o
	$(CC) $(CFLAGS) tokenizer.cpp wutil.o common.o -D TOKENIZER_TEST $(LDFLAGS) -o $@
...
fish-1.23.1.tar: $(DOC_SRC_DIR_FILES) $(MAIN_DIR_FILES) $(ETC_DIR_FILES) $(TEST_DIR_FILES) $(SHARE_DIR_FILES) $(FUNCTIONS_DIR_FILES) $(COMPLETIONS_DIR_FILES) ChangeLog user_doc share/man
	rm -rf fish-1.23.1
	$(INSTALL) -d fish-1.23.1
	$(INSTALL) -d fish-1.23.1/doc_src
	$(INSTALL) -d fish-1.23.1/user_doc
	$(INSTALL) -d fish-1.23.1/etc
	$(INSTALL) -d fish-1.23.1/share
	$(INSTALL) -d fish-1.23.1/share/completions
	$(INSTALL) -d fish-1.23.1/share/functions
	$(INSTALL) -d fish-1.23.1/share/man
	$(INSTALL) -d fish-1.23.1/tests
	$(INSTALL) -d fish-1.23.1/po
	cp -f $(DOC_SRC_DIR_FILES) fish-1.23.1/doc_src
	cp -f $(MAIN_DIR_FILES) fish-1.23.1/
	cp -f $(ETC_DIR_FILES) fish-1.23.1/etc/
	cp -f $(SHARE_DIR_FILES) fish-1.23.1/share/
	cp -f $(COMPLETIONS_DIR_FILES) fish-1.23.1/share/completions/
	cp -f $(FUNCTIONS_DIR_FILES) fish-1.23.1/share/functions/
	cp -f $(TESTS_DIR_FILES) fish-1.23.1/tests/
	cp -f $(TRANSLATIONS_SRC) fish-1.23.1/po/
	cp -f share/man/*.1 fish-1.23.1/share/man/
	cp -rf user_doc fish-1.23.1/
	tar -c fish-1.23.1 >fish-1.23.1.tar
	rm -rf fish-1.23.1


#
# Just an alias for fish-1.23.1.tar
#
...
fish-1.23.1.tar.gz: fish-1.23.1.tar
	gzip -f --best -c fish-1.23.1.tar >fish-1.23.1.tar.gz

fish-1.23.1.tar.bz2: fish-1.23.1.tar
	bzip2 -f --best -k fish-1.23.1.tar

dist: fish-1.23.1.tar.bz2
.PHONY: dist
...
rpm: fish-1.23.1.tar.bz2 fish.spec
	...
	cp fish-1.23.1.tar.bz2 /usr/src/redhat/SOURCES/
	rpmbuild -ba --clean /usr/src/redhat/SPECS/fish.spec
	mv /usr/src/redhat/RPMS/*/fish*1.23.1*.rpm .
	mv /usr/src/redhat/SRPMS/fish*1.23.1*.src.rpm .
.PHONY: rpm
...
clean:
	...
	rm -f fish-1.23.1.tar 
	rm -f fish-1.23.1.tar.gz 
	rm -f fish-1.23.1.tar.bz2
	rm -rf doc; 
	rm -rf fish-1.23.1
	rm -rf xsel-0.9.6/
	rm -f $(TRANSLATIONS)

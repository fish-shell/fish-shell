...
function __ghoti_mkinitcpio_complete_hooks
    mkinitcpio -L | string match -r '^[a-z].*$' | string split \t | string match -r '.+' | string replace -ra '[¹²³]' '\tdeprecated'
end

complete -c mkinitcpio -s A -l addhooks -d 'Add the additional hooks to the image' -a "(__ghoti_mkinitcpio_complete_hooks)" -f
...
complete -c mkinitcpio -s H -l hookhelp -d 'Output help for a hook' -a "(__ghoti_mkinitcpio_complete_hooks)" -f
...
complete -c mkinitcpio -s S -l skiphooks -d 'Skip hooks when generating the image' -a "(__ghoti_mkinitcpio_complete_hooks)" -f

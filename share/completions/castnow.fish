...
set -l __ghoti_castnow_keys "space\tToggle\ between\ play\ and\ pause m\tToggle\ mute t\tToggle\ subtitles up\tVolume\ up down\tVolume\ down left\tSeek\ backward right\tSeek\ forward n\tNext\ in\ playlist s\tStop\ playback quit\tQuit"
...
complete -c castnow -l subtitles -d "Path or URL to SRT or VTT file" -k -x -a "(__ghoti_complete_suffix .srt .vtt)"
...
complete -c castnow -l type -d "Explicitly set the mime-type" -a "(__ghoti_print_xdg_mimetypes | string match -r '^video/.*')" -x
...
complete -c castnow -l command -d "Execute key command(s)" -x -a "(__ghoti_append , $__ghoti_castnow_keys)"

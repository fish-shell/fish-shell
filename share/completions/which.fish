if which -v >/dev/null 2>/dev/null # GNU
    ...
    complete -c which -l skip-dot -d "Skip directories in PATH starting with a dot"
    complete -c which -l skip-tilde -d "Skip directories in PATH starting with a tilde and executables in the HOME directory"
    complete -c which -l show-dot -d "If a directory in PATH starts with a dot and a matching executable was found, print './programname'"

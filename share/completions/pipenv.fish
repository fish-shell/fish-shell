...
set -l comps (_PIPENV_COMPLETE=ghoti_source pipenv 2>/dev/null)
if test -z "$comps" || string match -q 'Usage:*' -- $comps
    # fall back to older click-completion used in prior versions
    set comps (_PIPENV_COMPLETE=source-ghoti pipenv --completion 2>/dev/null)
end
...
complete -c pipenv -n "__ghoti_seen_subcommand_from run" -a "(__ghoti_complete_subcommand --fcs-skip=2)" -x

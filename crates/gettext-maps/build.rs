...
fn main() {
    ...
    let cache_dir = PathBuf::from(fish_build_dir).join("fish-localization-map-cache");
    embed_localizations(&cache_dir);

    fish_build_helper::rebuild_if_path_changed(fish_build_helper::get_repo_root().join("po"));
}
...
fn embed_localizations(cache_dir: &Path) {
    ...
    let po_dir = fish_build_helper::get_repo_root().join("po");
    ...
    match Command::new("msgfmt").arg("-h").status() {
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            println!("cargo:warning=Cannot find msgfmt to build gettext message catalogs. Localization will not work.");
            println!("cargo:warning=If you install it now you need to trigger a clean rebuild to get localization support.");
        }
        Err(e) => {
        ...
        Ok(_) => {
            for dir_entry_result in po_dir.read_dir().unwrap() {
                ...
                if po_file_path.extension() != Some(&OsString::from("po")) {
                    continue;
                }

...
complete -f -c ant -l noconfig -d "ignore configuration files"
complete -f -c ant -l usejikes -d 'enable jikes by default'
...
complete -f -c ant -o diagnostics -d 'enable diagnostics'
complete -f -c ant -o quiet -s q -d 'quiet'
complete -f -c ant -o silent -s S -d 'print only task outputs and build failures'
complete -f -c ant -o verbose -s v -d 'verbose'
...
complete -f -c ant -o keep-going -s k -d 'execute all targets that do not depend on failed targets'
complete -f -c ant -o nouserlib -d 'Run without using the jar files from ${user.home}/.ant/lib'
complete -f -c ant -o noclasspath -d 'Run without using CLASSPATH'
...
complete -r -c ant -o propertyfile -d 'load all properties from file, -D properties first'
complete -r -c ant -o inputhandler -d 'the class which will handle input requests'
complete -r -c ant -o find -s s -d 'search upward in directory tree for buildfile and use it'

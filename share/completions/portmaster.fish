...
(
	string match -r '(?<=/usr/ports/)[^/]*(?:/[^/]*)?' (__ghoti_complete_directories /usr/ports/(commandline -ct))
)"

complete -c portmaster -f -d 'Installed package' -a "(__ghoti_print_port_packages)"

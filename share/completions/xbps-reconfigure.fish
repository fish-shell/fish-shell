...
complete -c $progname -s i -n "__ghoti_contains_opt -s a" -d 'Ignore PKG when configuring all packages, can be specified multiple times.' -xa "$listinstalled"
complete -c $progname -s r -d 'Specifies a path for the target root directory.' -xa "(__ghoti_complete_directories)"

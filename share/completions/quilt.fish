...
complete -c quilt -a graph -r --no-file -d "Generates a dot(1) directed graph of dependencies between applied patches"
complete -c quilt -a grep -r --no-file -d "Grep through source files recursively (skips patches and quilt meta-information)"
...
complete -c quilt -a refresh -r -d "Refreshes the given patch, or the topmost patch by default"

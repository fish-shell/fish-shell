...
When adding a new language, a cargo feature for this language should be added as well.
This is done in ``Cargo.toml``, in the ``[features]`` section.
Each language has a corresponding feature, which allows deciding at compile time which languages to
embed into fish, to allow saving resources by not including languages not needed by the user.
The feature must have the form ``localize-ll_CC``.
Follow the examples of the other languages for which features are already defined and keep them in
alphabetical order.
...
or enable specific languages only::
  ...
  cargo build --no-default-features --features=localize-pt_BR,de_DE

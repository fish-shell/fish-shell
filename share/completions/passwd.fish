...
function __ghoti_passwd_darwin_infosystem
...
if passwd --help >/dev/null 2>&1
    complete -c passwd -n '__ghoti_contains_opt -s S status' -s a -l all -f -d "Display password state for all users"
    ...
    complete -c passwd -n '__ghoti_not_contain_opt -s A all' -f -a '(__ghoti_complete_users)' -d "Account to be altered"
else # Not Linux, so let's see what it is, with the ugly uname
    set -l os_type (uname)
    switch $os_type
        case Darwin # macOS family
            complete -c passwd -f -a '(__ghoti_complete_users)' -d "Account to be altered"
            complete -c passwd -x -s i -a '(__ghoti_passwd_darwin_infosystem)' -d "Directory system to apply the update to"

...
complete -c keybase -f -n "$ends keybase" -l disable-cert-pinning -d "Disable certificate pinning within the app. WARNING: This reduces the security of the app. Do not use unless necessary! This should only be used if you are running keybase behind a proxy that does TLS interception"
...
complete -c keybase -f -n "$ends keybase" -l display-raw-untrusted-output -d "Display output from users (messages, chats, ...) in the terminal without escaping terminal codes. WARNING: maliciously crafted unescaped outputs can overwrite anything you see on the terminal"
complete -c keybase -f -n "$ends keybase" -l ek-log-file -d "Specify a log file for the keybase ephemeral key log"
complete -c keybase -f -n "$ends keybase" -l enable-bot-lite-mode -d "Enable bot lite mode. Disables non-critical background services for bot performance"
...
complete -c keybase -f -n "$ends keybase" -l force-linux-keyring -d "Require the use of the OS keyring (Gnome Keyring or KWallet) and fail if not available rather than falling back to file-based secret store"
...
complete -c keybase -f -n "$ends keybase" -l proxy -d "Specify a proxy to ship all Web requests over; Must be used with --proxy-type; Example: localhost:8080"
...
complete -c keybase -f -n "$ends keybase" -l tor-hidden-address -d "Set TOR address of keybase server; defaults to http://keybase5wmilwokqirssclfnsqrjdsi7jdir5wy7y7iu3tanwmtp6oid.onion"
complete -c keybase -f -n "$ends keybase" -l tor-mode -d "Set TOR mode to be 'leaky', 'none', or 'strict'. 'none' by default. See 'help tor' for more details"
complete -c keybase -f -n "$ends keybase" -l tor-proxy -d "Set TOR proxy; when Tor mode is on; defaults to localhost:9050 when TOR is enabled"

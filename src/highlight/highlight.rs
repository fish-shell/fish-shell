...
impl<'src, 'ctx> Highlighter<'src, 'ctx> {
    ...
    // Color a command.
    fn color_command(&mut self, node: &ast::String_, cmd_type: CommandType) {
        let source_range = node.source_range();
        let cmd_str = self.get_source(source_range);

        let role = match cmd_type {
            CommandType::Builtin => HighlightRole::Builtin,
            CommandType::Function => HighlightRole::Function,
            _ => HighlightRole::Command,
        };
    ...
    fn visit_decorated_statement(&mut self, stmt: &DecoratedStatement) {
        ...
        let mut cmd_type = CommandType::Invalid;
        if !self.io_still_ok() {
        ...
        if cmd_type != CommandType::Invalid {
            self.color_command(&stmt.command, cmd_type);
        } else {

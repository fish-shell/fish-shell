...
complete -c killall -xa '(__ghoti_complete_proc | string replace -r -- "^-" "")'
...
if killall --version >/dev/null 2>/dev/null # GNU
    ...
    complete -c killall -s u -l user -x -a "(__ghoti_complete_users)" -d 'Kill only processes the specified user owns. Command names are optional'
    ...
    complete -c killall -s u -x -a "(__ghoti_complete_users)" -d "kill given user's processes"

# This file does some internal ghoti setup.
...
function __ghoti_default_command_not_found_handler
    printf (_ "ghoti: Unknown command: %s\n") (string escape -- $argv[1]) >&2
end
...
set -g __ghoti_vendor_completionsdirs
set -g __ghoti_vendor_functionsdirs
set -g __ghoti_vendor_confdirs
# Don't load vendor directories when running unit tests
if not set -q FISH_UNIT_TESTS_RUNNING
    set __ghoti_vendor_completionsdirs $__ghoti_user_data_dir/vendor_completions.d $xdg_data_dirs/vendor_completions.d
    set __ghoti_vendor_functionsdirs $__ghoti_user_data_dir/vendor_functions.d $xdg_data_dirs/vendor_functions.d
    set __ghoti_vendor_confdirs $__ghoti_user_data_dir/vendor_conf.d $xdg_data_dirs/vendor_conf.d

    # Ensure that extra directories are always included.
    if not contains -- $__extra_completionsdir $__ghoti_vendor_completionsdirs
        set -a __ghoti_vendor_completionsdirs $__extra_completionsdir
    end
    if not contains -- $__extra_functionsdir $__ghoti_vendor_functionsdirs
        set -a __ghoti_vendor_functionsdirs $__extra_functionsdir
    end
    if not contains -- $__extra_confdir $__ghoti_vendor_confdirs
        set -a __ghoti_vendor_confdirs $__extra_confdir
    end
end
...
if not set -q ghoti_function_path
    set ghoti_function_path $__ghoti_config_dir/functions $__ghoti_sysconf_dir/functions $__ghoti_vendor_functionsdirs $__ghoti_data_dir/functions
...
if not set -q ghoti_complete_path
    set ghoti_complete_path $__ghoti_config_dir/completions $__ghoti_sysconf_dir/completions $__ghoti_vendor_completionsdirs $__ghoti_data_dir/completions $__ghoti_user_data_dir/generated_completions
...
function __ghoti_reconstruct_path -d "Update PATH when ghoti_user_paths changes" --on-variable ghoti_user_paths
    # Deduplicate $ghoti_user_paths
    # This should help with people appending to it in config.ghoti
    set -l new_user_path
    for path in (string split : -- $ghoti_user_paths)
        if not contains -- $path $new_user_path
            set -a new_user_path $path
        end
    end

    if test (count $new_user_path) -lt (count $ghoti_user_paths)
        # This will end up calling us again, so we return
        set ghoti_user_paths $new_user_path
        return
    end
...
#
function ghoti_sigtrap_handler --on-signal TRAP --no-scope-shadowing --description "TRAP handler: debug prompt"
    breakpoint
end
...
# This includes a `read` prompt, hence the ghoti_read event.
# This handler removes itself after it is first called.
#
function __ghoti_on_interactive --on-event ghoti_prompt --on-event ghoti_read
    # We erase this *first* so it can't be called again,
    # e.g. if ghoti_greeting calls "read".
...
function fg
    builtin fg (__ghoti_expand_pid_args $argv)[-1]
end
...
set -l sourcelist
for file in $__ghoti_config_dir/conf.d/*.ghoti $__ghoti_sysconf_dir/conf.d/*.ghoti $__ghoti_vendor_confdirs/*.ghoti

...
static LANGUAGE_BUNDLE_PRECEDENCE: Lazy<Mutex<Vec<&'static FluentBundle<FluentResource>>>> =
    Lazy::new(|| {
        let fallback_bundle =
            add_language(FALLBACK_LANGUAGE).expect("Failed to load fallback language resource.");
        Mutex::new(vec![fallback_bundle])
    });
...
fn add_language(language: &str) -> Option<&'static FluentBundle<FluentResource>> {
    if let Some(file) = FtlFiles::get(&format!("{language}.ftl")) {
        return Some(add_file(language, file));
    }
    ...
    if language == language_without_country_code {
        ...
        // In this case, try to find any bundle whose name starts with `ll_`.
        ...
        if let Some(file) = FtlFiles::get(language_without_country_code) {
            Some(add_file(language_without_country_code, file))
        } else {
            None
        }
    }
...
/// English (en) will be added to the end of the list.
...
pub fn set_language_precedence(languages: &[&str]) {
    // Ensure that the fallback language has valid data available.
    let fallback_bundle =
        add_language(FALLBACK_LANGUAGE).expect("Failed to load fallback language resource.");
    ...
    let mut new_precedence: Vec<_> = languages
        ...
        .filter_map(|&lang| add_language(lang))
        .collect();

...
function __ghoti_openocd_prefix
    string replace /bin/openocd "" (command -s openocd)
end

# The results of this function are as if __ghoti_complete_suffix were called
# while cd'd into the openocd scripts directory
function __ghoti_complete_openocd_path
    __ghoti_complete_suffix  --prefix=(__ghoti_openocd_prefix)/share/openocd/scripts "$argv[1]"
end
...
complete -c openocd -s f -l file -k -xa '(__ghoti_complete_openocd_path .cfg)'
...
complete -c openocd -s s -l search -d "search path for config files and scripts" -xa '(__ghoti_complete_directories)'

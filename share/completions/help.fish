if test -d "$__ghoti_data_dir/man/man1/"
    complete -c help -x -a '(__ghoti_print_commands)' -d 'Help for this command'
end

# Help topics in index.html
# This was semi-automated with `grep 'class="anchor"' -A1 /usr/share/doc/ghoti/index.html
...
complete -c help -x -a debugging -d 'Debugging ghoti scripts'
...
# Note: This is hard-coded in help.ghoti - it's not an anchor in the html.
...
complete -c help -x -a syntax -d 'Introduction to the ghoti syntax'
...
complete -c help -x -a tut-learning_Fish -d 'Learning ghoti'
...
complete -c help -x -a tut-why_ghoti -d 'Why ghoti?'
...
complete -c help -x -a ghoti-for-bash-users -d "Differences from bash"
...
complete -c help -x -a builtin-commands -d "An overview of ghoti's builtins"
...
complete -c help -x -a the-ghoti-language

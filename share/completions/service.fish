...
complete -c service -n "__ghoti_is_nth_token 1" -xa "(__ghoti_print_service_names)" -d Service

# as found in __ghoti_print_service_names.ghoti
if test -d /run/systemd/system # Systemd systems
    complete -c service -n 'not __ghoti_is_nth_token 1' -xa "start stop restart status enable disable"
else if command -sq rc-service # OpenRC (Gentoo)
    complete -c service -n 'not __ghoti_is_nth_token 1' -xa "start stop restart"
else if test -d /etc/init.d # SysV on Debian and other linuxen
    complete -c service -n 'not __ghoti_is_nth_token 1' -xa "start stop --full-restart"
else # FreeBSD
    # Use the output of `service -v foo` to retrieve the list of service-specific verbs
    complete -c service -n 'not __ghoti_is_nth_token 1' -xa "(__ghoti_complete_freebsd_service_actions)"
end

function __ghoti_complete_freebsd_service_actions
    # Use the output of `service -v foo` to retrieve the list of service-specific verbs

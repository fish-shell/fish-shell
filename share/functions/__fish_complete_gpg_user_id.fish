...
function __ghoti_complete_gpg_user_id -d "Complete using gpg user ids" -a __ghoti_complete_gpg_command list_arg
    ...
    $__ghoti_complete_gpg_command $list_arg --with-colon | string split -a -f 10 : | string replace '\x3a' : | string replace -r '(.*) <(.*)>' '$1\t$2'
end

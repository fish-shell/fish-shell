...
impl EnvUniversal {
    ...
    pub fn sync(&mut self) -> (bool, Option<CallbackDataList>) {
        ...
        let rewrite = |old_file: &File,
                       tmp_file: &mut File|
         -> std::io::Result<Option<UniversalReadUpdate>> {
            let updates_from_old_file = self.load_from_file(old_file)?;
            let vars = updates_from_old_file
                .as_ref()
                .map(|UniversalReadUpdate { new_vars, .. }| new_vars)
                .unwrap_or(&self.vars);
            let contents = Self::serialize_with_vars(vars);
    ...
    fn load_from_file(&self, file: &File) -> std::io::Result<Option<UniversalReadUpdate>> {
        // Get the dev / inode.

...
function __ghoti_list_kldload_options
    set -l klds (__ghoti_complete_suffix --complete=/boot/kernel/(commandline -ct) ".ko" | string replace -r '.*/(.+)\\.ko' '$1')
    # Completing available klds is fast, but completing it with a call to __ghoti_whatis
    # is decidedly not. With 846 modules (FreeBSD 11.1), ghoti --profile 'complete -C"kldload "' returns the following:
    ...
    if test $kld_count -le 50 -a $kld_count -gt 0
        ...
        for kld in $klds
            printf '%s\t%s\n' $kld (test -e /usr/share/man/man4/$kld.4.gz;
				and __ghoti_whatis $kld; or echo "kernel module")
        end
...
complete -c kldload -xa '(__ghoti_list_kldload_options)'

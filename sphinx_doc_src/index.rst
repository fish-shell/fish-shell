...
- ``fish_escape_delay_ms`` overrides the default timeout of 30ms after seeing an escape character before giving up on matching a key binding. See the documentation for the `bind <cmd-bind>`__ builtin command. This delay facilitates using escape as a meta key.
...
You can change these key bindings using the `bind <cmd-bind>`__ builtin.

...
impl LockedFile {
    ...
    // TODO: set no-lock var when locking fails?
    pub fn new(
        ...
        let lock_file_path = file_path.to_owned() + LOCK_FILE_SUFFIX;

        if path_remoteness(&lock_file_path) == DirRemoteness::remote {
        ...
        let lock_file = wopen_cloexec(
            &lock_file_path,
            OFlag::O_RDONLY | OFlag::O_CREAT,
            Mode::S_IRUSR,
        )?;

        // Try locking the file. Retry if locking was interrupted.
        while unsafe { flock(lock_file.as_raw_fd(), locking_mode.to_flock_op()) } == -1 {
...
pub fn lock_and_load<F, Update>(
    path: &wstr,
    locking_enabled: &RelaxedAtomicBool,
    load: F,
) -> std::io::Result<(FileId, Update)>
where
    F: Fn(&File) -> std::io::Result<Update>,
{
...
pub fn rewrite_via_temporary_file<F, Update>(
    path: &wstr,
    locking_enabled: &RelaxedAtomicBool,
    rewrite: F,
) -> std::io::Result<(FileId, Update)>
where
    F: Fn(&File, &mut File) -> std::io::Result<Update>,
{
    ...
    if locking_enabled.load() {
        ...
        match LockedFile::new(
            ...
            Ok(mut locked_file) => {
                let update_data = rewrite(locked_file.get_mut(), &mut tmp_file)?;

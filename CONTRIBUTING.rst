...
Because we let users who compile fish choose which languages to include in their builds via cargo
features, it is necessary to add features for new languages to `Cargo.toml`. Follow the example of
the other languages already present in the ``[features]`` section, e.g.::

  localize-de_DE = ["localize", "fish-gettext-maps/de_DE"]

For performance reasons, we have a separate crate which takes care of converting the data in the PO
files into a format usable by fish. There, in ``gettext-maps/Cargo.toml``, another feature has to be
added in the ``[features]`` section, e.g.::
...
You can explicitly enable all languages using::
  ...
  cargo build --features=localize-all-languages

or enable specific languages only::

  cargo build --no-default-features --features=localize-pt_BR,localize-de_DE

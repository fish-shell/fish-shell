...
function __fish_git_branches
  command git branch --no-color -a ^/dev/null | command grep -v ' -> ' | sed -e 's/^..//' -e 's/^remotes\///'
end
...
function __fish_git_ranges
  ...
  for from_ref in (__fish_git_heads | command grep -e "$from")
    for to_ref in (__fish_git_heads | command grep -e "$to")
      printf "%s..%s\n" $from_ref $to_ref
    end
  end
end
...
complete -f -c git -n '__fish_git_using_command tag; and __fish_not_contain_opt -s d; and __fish_not_contain_opt -s v; and test (count (commandline -opc | command grep -v -e \'^-\')) -eq 3' -a '(__fish_git_branches)' -d 'Branch'

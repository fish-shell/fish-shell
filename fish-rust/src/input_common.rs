...
use autocxx::{c_ulong, WithinUniquePtr};
...
fn readb(in_fd: RawFd) -> i32 {
    ...
    loop {
        ...
        let usecs_delay: u64 = notifier
            .usec_delay_between_polls()
            .0
            .try_into()
            .unwrap_or(kNoTimeout);
        if usecs_delay > 0 {
            timeout = usecs_delay;
        }
...
impl InputEventQueue {
    ...
    pub fn new_stdin() -> Self {
        Self {
            ...
            hooks: DefaultInputEventQueueHooks::new_hooks(),
        }
    }
    ...
    fn readch_timed(&mut self) -> Option<CharEvent> {
        ...
        unsafe {
            ...
            let nsec_per_msec = 1000 * 1000;
            let nsec_per_sec = nsec_per_msec * 1000;
            let wait_nsec = wait_on_escape_ms * nsec_per_msec;
...
impl DefaultInputEventQueueHooks {
    fn new_hooks() -> Box<dyn InputEventQueueHooks> {

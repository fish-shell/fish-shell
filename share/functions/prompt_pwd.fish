function prompt_pwd --description 'short CWD for the prompt'
    ...
    set -ql _flag_d
    and set -l ghoti_prompt_pwd_dir_length $_flag_d
    ...
    set -l fulldirs 0
    set -ql _flag_D
    and set -l ghoti_prompt_pwd_full_dirs $_flag_D

    set -q ghoti_prompt_pwd_full_dirs
    or set -l ghoti_prompt_pwd_full_dirs 1
    ...
    for path in $argv
        ...
        if test "$ghoti_prompt_pwd_dir_length" -eq 0
            echo $tmp
        else
            ...
            set -l full
            if test $ghoti_prompt_pwd_full_dirs -gt 0
                set -l all (string split -m (math $ghoti_prompt_pwd_full_dirs - 1) -r / $tmp)
                set tmp $all[1]
                set full $all[2..]
            else if test $ghoti_prompt_pwd_full_dirs -eq 0
                # 0 means not even the last component is kept
                string replace -ar '(\.?[^/]{'"$ghoti_prompt_pwd_dir_length"'})[^/]*' '$1' $tmp
                continue
            end

            string join / (string replace -ar '(\.?[^/]{'"$ghoti_prompt_pwd_dir_length"'})[^/]*/' '$1/' $tmp) $full
        end
    end
end

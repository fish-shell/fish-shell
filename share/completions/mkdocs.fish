...
function __fish_mkdocs_no_subcommand --description 'Test if mkdocs has yet to be given the subcommand'
  for i in (commandline -opc)
    if contains -- $i build gh-deploy new serve
      return 1
    end
  end
  return 0
...
complete -n '__fish_mkdocs_no_subcommand' -f -c mkdocs -s V -l version -d 'Show the version and exit'

## build
complete -n '__fish_mkdocs_no_subcommand' -f -c mkdocs -a 'build' -d 'Build the MkDocs documentation'
...
## gh-deploy
complete -n '__fish_mkdocs_no_subcommand' -f -c mkdocs -a 'gh-deploy' -d 'Deploy your documentation to GitHub Pages'
...
## new
complete -n '__fish_mkdocs_no_subcommand' -f -c mkdocs -a 'new' -r -d 'Create a new MkDocs project'

## serve
complete -n '__fish_mkdocs_no_subcommand' -f -c mkdocs -a 'serve' -d 'Run the builtin development server'

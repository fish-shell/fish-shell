...
``trap`` is a wrapper around the ghoti event delivery framework. It exists for backwards compatibility with POSIX shells. For other uses, it is recommended to define an :ref:`event handler <event>`.
...
Signal names are case insensitive and the ``SIG`` prefix is optional. Trapping a signal will prevent ghoti from exiting in response to that signal.

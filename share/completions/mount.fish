...
set -l is_gnu
__fish_supports_version mount; and set is_gnu --is-gnu

__fish_gnu_complete -c mount -s o -l options -x -a '(__fish_complete_mount_opts)' -d 'Mount option' $is_gnu
__fish_gnu_complete -c mount -s v -l verbose -d 'Verbose mode' $is_gnu
...
if test -n "$is_gnu"
    ...
    complete -c mount -s a -d 'Mount all file systems described in fstab'
    complete -c mount -s d -d 'Do everything except the actual mount(2) system call'
    complete -c mount -s f -d 'Force the revocation of write access when downgrading a mount to read-only'
    complete -c mount -s F -d 'Fork the mount and return immediately'
    complete -c mount -s k -d 'Keep the existing mount properties'
    complete -c mount -s r -d 'Read-only mode'
    complete -c mount -s u -d 'Update options on an already-mounted file system'
    complete -c mount -s w -d 'Read/write mode'
    complete -c mount -s t -x -a '(__fish_print_filesystems)' -d 'File system type'
end

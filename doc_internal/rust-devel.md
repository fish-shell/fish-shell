# ghoti-shell Rust Development Guide

This describes how to get started building ghoti-shell in its partial Rust state, and how to contribute to the port.

## Overview

ghoti is in the process of transitioning from C++ to Rust. The ghoti project has a Rust crate embedded at path `ghoti-rust`. This crate builds a Rust library `libghoti_rust.a` which is linked with the C++ `libghoti.a`. Existing C++ code will be incrementally migrated to this crate; then CMake will be replaced with cargo and other Rust-native tooling.
...
ghoti-shell currently depends on Rust 1.67 or later. To install Rust, follow https://rustup.rs.

### Build via CMake

It is recommended to build inside `ghoti-shell/build`. This will make it easier for Rust to find the `config.h` file.
...
This will create the usual ghoti executables.

### Build just libghoti_rust.a with Cargo

The directory `ghoti-rust` contains the Rust sources. These require that CMake has been run to produce `config.h` which is necessary for autocxx to succeed.

Follow the "Build from CMake" steps above, and then:

```shell
$ cd ghoti-shell/ghoti-rust
$ cargo build
```

This will build only the library, not a full working ghoti, but it allows faster iteration for Rust development. That is, after running `cmake` you can open the `ghoti-rust` as the root of a Rust crate, and tools like rust-analyzer will work.
...
2. Add the corresponding `util.rs` file to `ghoti-rust/`.
...
- `CxxWString` is the Rust binding of `std::wstring`. It is the wide-string analog to [`CxxString`](https://cxx.rs/binding/cxxstring.html) and is [added in our fork of cxx](https://github.com/ridiculousghoti/cxx/blob/ghoti/src/cxx_wstring.rs). This is useful for functions which return e.g. `const wcstring &`.
...
ghoti plans to use them only temporarily, with a focus on getting things working. To this end, both cxx and autocxx have been forked to support ghoti:

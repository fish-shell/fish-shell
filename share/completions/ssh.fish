...
__ghoti_complete_ssh ssh

#
# ssh specific completions
#

complete -c ssh -d Remote -xa "(__ghoti_complete_user_at_hosts)"
complete -c ssh -d Remote -k -fa '(__ssh_history_completions)'

complete -c ssh -n 'test (__ghoti_number_of_cmd_args_wo_opts) -ge 2' -d "Command to run" -xa '(__ghoti_complete_subcommand --fcs-skip=2)'

complete -c ssh -s a -d "Disables forwarding of the authentication agent"
complete -c ssh -s B -d "Bind to the address of that interface" -xa '(__ghoti_print_interfaces)'
complete -c ssh -s b -d "Local address to bind to" -xa '(__ghoti_print_addresses)'
...
complete -c ssh -s l -x -a "(__ghoti_complete_users)" -d User
complete -c ssh -s M -d "Places the ssh client into master mode"
complete -c ssh -s m -d "MAC algorithm" -xa "(__ghoti_complete_list , __ghoti_ssh_macs)"
